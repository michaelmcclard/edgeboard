...
interface PowerParlay {   legs: BestBet[];   combinedOdds: number;   impliedProb: number;   modelProb: number;   edgePct: number;   isBet: boolean;   updatedAt: Date; }  function buildPowerParlay(bets: BestBet[]): PowerParlay | null {   if (bets.length < 3) return null;   const sorted = [...bets].sort((a, b) => (b.confidence || 0) - (a.confidence || 0));   const legs: BestBet[] = [];   const usedGames = new Set<string>();   const usedTypes = new Set<string>();   // First pass: pick highest confidence non-correlated legs, prefer type diversity   for (const b of sorted) {     if (legs.length >= 3) break;     if (usedGames.has(b.game_id)) continue;     const typeGroup = b.bet_type === 'moneyline' || b.bet_type === 'spread' || b.bet_type === 'run_line' || b.bet_type === 'puck_line' ? 'side' : b.bet_type === 'total' || b.bet_type === 'first_5' ? 'total' : 'prop';     if (legs.length >= 2 && usedTypes.has(typeGroup) && sorted.some(x => !usedGames.has(x.game_id) && x.confidence >= 7 && ((x.bet_type === 'moneyline' || x.bet_type === 'spread' || x.bet_type === 'run_line' || x.bet_type === 'puck_line' ? 'side' : x.bet_type === 'total' || x.bet_type === 'first_5' ? 'total' : 'prop') !== typeGroup))) continue;     legs.push(b);     usedGames.add(b.game_id);     usedTypes.add(typeGroup);   }   // Fallback: fill remaining legs without type constraint   if (legs.length < 3) {     for (const b of sorted) {       if (legs.length >= 3) break;       if (usedGames.has(b.game_id)) continue;       legs.push(b);       usedGames.add(b.game_id);     }   }   if (legs.length < 3) return null;   const finalLegs = legs.slice(0, 3);   // Calculate combined American odds from individual model probabilities   const legProbs = finalLegs.map(l => Math.min(0.85, (l.confidence / 10) * 0.9));   const modelProb = legProbs.reduce((a, b) => a * b, 1);   const impliedProb = modelProb * 0.82; // Book vig adjustment   const decimalOdds = 1 / impliedProb;   const americanOdds = decimalOdds >= 2 ? Math.round((decimalOdds - 1) * 100) : Math.round(-100 / (decimalOdds - 1));   const edgePct = parseFloat(((modelProb - impliedProb) / impliedProb * 100).toFixed(1));   return {     legs: finalLegs,     combinedOdds: americanOdds,     impliedProb: parseFloat((impliedProb * 100).toFixed(1)),     modelProb: parseFloat((modelProb * 100).toFixed(1)),     edgePct,     isBet: modelProb >= 0.35,     updatedAt: new Date()   }; }  function generateParlayLegReason(b: BestBet): string {   const sport = b.sport;   const betType = b.bet_type;   const hp = b.home_pitcher;   const ap = b.away_pitcher;   const hg = b.home_goalie;   const ag = b.away_goalie;   if (sport === 'MLB' && betType === 'player_prop' && b.pick.toLowerCase().includes('strikeout')) {     const p = hp?.confirmed ? hp : ap?.confirmed ? ap : null;     if (p) return `${p.name} owns a ${p.kPer9.toFixed(1)} K/9 against a lineup that strikes out at an elite clip.`;   }   if (sport === 'MLB' && betType === 'moneyline') {     const fav = hp && ap ? (hp.era < ap.era ? hp : ap) : hp || ap;     const dog = hp && ap ? (hp.era < ap.era ? ap : hp) : null;     if (fav && dog) return `${fav.name} (${fav.era.toFixed(2)} ERA) has a decisive pitching edge over ${dog.name} (${dog.era.toFixed(2)} ERA).`;   }   if (sport === 'MLB' && (betType === 'total' || betType === 'first_5')) {     return `Combined pitching profile and park factor align for this ${betType === 'first_5' ? 'F5' : ''} total.`;   }   if (sport === 'MLB' && (betType === 'run_line' || betType === 'spread')) {     const fav = hp && ap ? (hp.era < ap.era ? hp : ap) : hp || ap;     if (fav) return `${fav.name} goes deep into games and the bullpen edge supports covering -1.5.`;   }   if (sport === 'NHL') {     const better = hg && ag ? (hg.savePct > ag.savePct ? hg : ag) : hg || ag;     if (better) return `${better.name} (${(better.savePct*100).toFixed(1)}% SV) gives a clear goaltending advantage.`;   }   if (sport === 'NBA') {     return `Defensive matchup data and pace projection both favor this side at current numbers.`;   }   return `${b.edge_pct.toFixed(1)}% edge cleared the data gate with high confidence.`; }  function generateParlayCloser(parlay: PowerParlay): string {   const types = new Set(parlay.legs.map(l => l.bet_type));   const sports = new Set(parlay.legs.map(l => l.sport));   const avgConf = (parlay.legs.reduce((s, l) => s + l.confidence, 0) / parlay.legs.length).toFixed(1);   const diverseStr = types.size >= 2 ? `${types.size} different bet types` : 'focused approach';   const sportStr = sports.size >= 2 ? `across ${sports.size} sports` : `in ${[...sports][0]}`;   if (!parlay.isBet) return `Combined implied probability dropped below 35%. This parlay is flagged NO BET today — the individual plays are stronger on their own.`;   return `Three independently validated edges ${sportStr}, ${diverseStr}, zero correlated risk. Average leg confidence: ${avgConf}/10. Model gives this a ${parlay.modelProb}% hit rate against ${parlay.impliedProb}% implied. This is the play.`; }  function Card({ title, icon, children, className = "" }: { title: string; icon: React.ReactNode; children: React.ReactNode; className?: string }) {
  return (
...
export default function App() {
  ...
  const top5 = [...bets].sort((a, b) => (b.edge_pct || 0) - (a.edge_pct || 0)).slice(0, 5);     const powerParlay = buildPowerParlay(bets);
  const betCount = {
  ...
  return (
    <div className="min-h-screen bg-edge-bg text-edge-muted p-4 max-w-7xl mx-auto">
      ...
      {/* DAILY POWER PARLAY */}       {powerParlay && (         <div className="mb-8">           <div className="relative rounded-2xl overflow-hidden" style={{background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #1a1a2e 100%)', border: '2px solid transparent', backgroundClip: 'padding-box'}}>             <div className="absolute inset-0 rounded-2xl" style={{background: 'linear-gradient(135deg, #f59e0b, #d97706, #b45309, #d97706, #f59e0b)', mask: 'linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)', maskComposite: 'xor', WebkitMaskComposite: 'xor', padding: '2px', borderRadius: '1rem'}} />             <div className="relative p-6">               <div className="flex items-center justify-between mb-4">                 <div className="flex items-center gap-3">                   <div className="p-2 rounded-lg bg-amber-500/20"><Trophy className="w-6 h-6 text-amber-400" /></div>                   <div>                     <h2 className="text-lg font-black text-amber-400 tracking-wide uppercase">Daily Power Parlay</h2>                     <p className="text-[10px] text-amber-600/80 uppercase tracking-widest">3-Leg • Highest Combined Confidence • Zero Correlation</p>                   </div>                 </div>                 <div className="text-right">                   <div className="text-3xl font-black text-amber-400 font-mono">+{powerParlay.combinedOdds}</div>                   <div className="text-[10px] text-edge-muted mt-0.5">                     {powerParlay.impliedProb}% implied / {powerParlay.modelProb}% model                   </div>                 </div>               </div>               {!powerParlay.isBet && (                 <div className="mb-4 px-3 py-2 rounded-lg bg-red-900/30 border border-red-800/50 text-red-400 text-xs font-semibold flex items-center gap-2">                   <AlertTriangle className="w-4 h-4" /> NO BET — Combined implied probability below 35%. Individual legs are stronger plays today.                 </div>               )}               <div className="space-y-3 mb-4">                 {powerParlay.legs.map((leg, i) => (                   <div key={leg.id} className="flex items-start gap-3 p-3 rounded-xl bg-black/30 border border-amber-900/30">                     <div className="mt-0.5"><Lock className="w-4 h-4 text-amber-500" /></div>                     <div className="flex-1 min-w-0">                       <div className="flex items-center gap-2 mb-1 flex-wrap">                         <SportBadge sport={leg.sport} />                         <BetTypeBadge type={leg.bet_type} />                         <span className="text-white font-bold text-sm">{leg.pick}</span>                         <span className="text-amber-400 font-mono text-xs font-bold ml-auto">{leg.confidence.toFixed(1)}</span>                       </div>                       <p className="text-[11px] text-edge-muted leading-snug">{generateParlayLegReason(leg)}</p>                     </div>                   </div>                 ))}               </div>               <div className="border-t border-amber-900/30 pt-4">                 <div className="flex items-center gap-4 text-xs mb-3 flex-wrap">                   <span className="text-edge-muted">Combined Odds: <span className="text-amber-400 font-bold font-mono">+{powerParlay.combinedOdds}</span></span>                   <span className="text-edge-muted">Model: <span className="text-edge-green font-bold">{powerParlay.modelProb}%</span></span>                   <span className="text-edge-muted">Book Implied: <span className="text-edge-amber font-bold">{powerParlay.impliedProb}%</span></span>                   <span className="text-edge-muted">Edge: <span className="text-edge-green font-bold">+{powerParlay.edgePct}%</span></span>                 </div>                 <p className="text-xs text-amber-200/70 italic leading-relaxed">{generateParlayCloser(powerParlay)}</p>                 <p className="text-[9px] text-edge-muted mt-2">Updated {powerParlay.updatedAt.toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'})}</p>               </div>             </div>           </div>         </div>       )}        <Card title="TODAY'S GAMES" icon={<BarChart3 size={16} className="text-edge-green" />} className="lg:col-span-2">
